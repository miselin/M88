...
static int do_div(long __ss *n, int base) {
  long v = *n;
  long __res = 0;
  asm {
    mov ax, v
...
// For IDIV r/m16, DX:AX is divided by the given operand; the quotient is stored
// in AX and the remainder in DX.

static char __ss *number(char __ss *str, long num, int base, int size,
...
int vsprintf(char __ss *buf, const char *fmt, va_list args) {
  int len;
  int i;
  char __ss *str;
  char *s;
